...
long ADS1219::readDifferential_0_1(){ //modified and changed to just send config command for 330Hz sampling, internal ref and single shot mode
  //config &= MUX_MASK;
  //config |= MUX_DIFF_0_1;
  //writeRegister(config);
  writeRegister(0x18);
...
void ADS1219::defaultSetup(){
  writeRegister(0x1E); //A
}
